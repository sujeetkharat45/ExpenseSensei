...
export default function RegisterScreen({ navigation }) {
  ...
  const handleRegister = async () => {
    ...
    if (!name || !email || !password || !mobile) return Alert.alert("Error", "All fields required");
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingVertical: 60 }}>
      ...
      <TextInput placeholder="Name" placeholderTextColor="#aaa" style={styles.input} value={form.name} onChangeText={(v) => setForm({ ...form, name: v })} />
      <TextInput placeholder="Email" placeholderTextColor="#aaa" style={styles.input} value={form.email} onChangeText={(v) => setForm({ ...form, email: v })} keyboardType="email-address" autoCapitalize="none" />
      <TextInput placeholder="Mobile" placeholderTextColor="#aaa" style={styles.input} value={form.mobile} onChangeText={(v) => setForm({ ...form, mobile: v })} keyboardType="phone-pad" />
      
      <View style={styles.genderRow}>
        {["Male", "Female"].map(g => (
          <TouchableOpacity key={g} style={[styles.genderBtn, form.gender === g && styles.activeGender]} onPress={() => setForm({ ...form, gender: g })}>
      ...
      <View style={styles.passwordWrapper}>
        <TextInput placeholder="Password" placeholderTextColor="#aaa" secureTextEntry={!passwordVisible} style={styles.passwordInput} value={form.password} onChangeText={(v) => setForm({ ...form, password: v })} />
      ...
      <TouchableOpacity onPress={suggestPassword}><Text style={styles.suggestText}>Suggest Strong Password</Text></TouchableOpacity>

      <TouchableOpacity style={styles.button} onPress={handleRegister} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Register</Text>}
      </TouchableOpacity>
      
      <SuccessModal visible={showSuccess} title="Success" message="Account Created!" onConfirm={() => { setShowSuccess(false); login(); }} />
    </ScrollView>
  );
}
